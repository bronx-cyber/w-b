/* ============================================================
   BRONX ULTRA v5.6 · Simple Gmail Login OSINT Panel
   @BRONX_ULTRA · Render.com Ready · Zero Errors
   ============================================================ */
...
/* ============================================================
   Broadcast JS (safe · no regex issues)
   ============================================================ */
const BROADCAST_JS = [
  ...
  '      var txt = String(j.message).split("<").join("&lt;");',
  '      d.textContent = "📢 " + j.message;',
...
/* ============================================================
   HTML · LOGIN (Simple Gmail + Password)
   ============================================================ */
...
<div class="center">
  <div class="box">
    ...
    <div class="info" style="margin-top:14px">
      Sirf <b style="color:#22d3ee">@gmail.com</b> allowed. Pehli baar login = account auto-create. Dobara same Gmail + same Password = same data wapas.
    </div>
...
function doLogin(){
  ...
  if (p.length < 1){ showErr('Password required'); return; }
...
<footer>© BRONX ULTRA · @BRONX_ULTRA · v5.6</footer>
...
<div class="container">
  ...
  <div class="section">
    ...
    <div class="success" id="success">
      ...
      <div class="warn">⏳ <b>Please wait — Admin Approval Pending</b><br/>Aapka payment admin ke paas verify ke liye chala gaya. Approve hone ke baad aapka ye token activate ho jayega aur aap apne Gmail account se login karke saare features use kar payenge.<br/>Support: <a href="${esc(CFG.TG_LINK)}" target="_blank" style="color:#22d3ee">@BRONX_ULTRA</a></div>
...
<div class="wrap">
  <header>
    <div class="brand">
      <div class="logo2">⚙</div>
      <div><h1><span>BRONX</span> ADMIN</h1><div class="sub2">@BRONX_ULTRA · v5.6</div></div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap">
  ...
  <div id="paneFeatures" style="display:none">
    <div class="card">
      ...
      <p style="font-size:11px;color:#94a3b8;margin-bottom:12px">API key env me hai — frontend pe expose nahi hoti.</p>
      <div id="featList"></div>
...
function showTab(t){
  var names = ['pay', 'tokens', 'users', 'features', 'plans', 'broadcast', 'settings'];
...
function renderHist(list){
  ...
  for (var i = 0; i < list.length; i++){
    ...
    h += '<div style="padding:10px;border-radius:10px;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.06);margin-bottom:6px;opacity:.85">'
      ...
      + '<div style="font-size:11px;color:#94a3b8;margin-top:4px">' + escapeHTML(p.email || '—') + ' · TG: ' + escapeHTML(p.tg || '—') + ' · ' + fmtDate(p.processedAt) + '</div>'
      + '</div>';
  }
  el.innerHTML = h;
}
...
function copyText(t){
  if (navigator.clipboard) navigator.clipboard.writeText(t).then(function(){ alert('Copied: ' + t); });
}
...
function renderFeatures(){
  ...
  for (var i = 0; i < FEATURES.length; i++){
    ...
    h += '<div class="feat-row">'
      + '<input value="' + escapeHTML(f.emoji || '') + '" data-i="' + i + '" data-k="emoji" placeholder="emoji"/>'
      + '<input value="' + escapeHTML(f.name || '') + '" data-i="' + i + '" data-k="name" placeholder="Name"/>'
      + '<input value="' + escapeHTML(f.color || '#22d3ee') + '" data-i="' + i + '" data-k="color" placeholder="color"/>'
      + '<input value="' + escapeHTML(f.endpoint || '') + '" data-i="' + i + '" data-k="endpoint" placeholder="/api/key-bronx/xxx"/>'
      + '<input value="' + escapeHTML(f.param || 'num') + '" data-i="' + i + '" data-k="param" placeholder="param"/>'
  ...
  for (var j = 0; j < inputs.length; j++){
    inputs[j].addEventListener('input', function(ev){
      var i = parseInt(ev.target.dataset.i), k = ev.target.dataset.k;
      FEATURES[i][k] = ev.target.value;
    });
  }
}
function addFeature(){
...
function saveFeatures(){
  fetch('/api/admin/features/save', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ features: FEATURES }) })
    .then(function(r){ return r.json(); }).then(function(j){ alert(j.ok ? '✓ Saved' : '✗ Failed'); });
}
...
function renderPlans(){
  ...
  for (var i = 0; i < PLANS.length; i++){
    ...
    h += '<div class="plan-row">'
      + '<input value="' + escapeHTML(p.label || '') + '" data-i="' + i + '" data-k="label" placeholder="Label"/>'
      ...
      + '<input value="' + escapeHTML(p.tag || '') + '" data-i="' + i + '" data-k="tag" placeholder="Tag"/>'
...
function killAllSessions(){
  ...
  fetch('/api/admin/kill-sessions', { method: 'POST' })
    .then(function(r){ return r.json(); }).then(function(j){ if (j.ok) alert('✓ Done'); });
}
function logout(){
...
/* ============================================================
   GMAIL LOGIN (Simple · any password)
   ============================================================ */
app.post('/api/auth/gmail', async (req, res) => {
  ...
  /* Returning user: password must match (if already set) */
  ...
  if (!user.password){
    /* Legacy: no password set — save it now */
...
async function attachDevice(user, uid, fp, ip, device, ua){
  ...
  var existing = null;
  for (var i = 0; i < user.devices.length; i++){ if (user.devices[i].fp === fp){ existing = user.devices[i]; break; } }
  if (existing){
...
app.get('/api/lookup', async (req, res) => {
  ...
  var feat = null;
  for (var i = 0; i < features.length; i++){ if (features[i].id === featureId){ feat = features[i]; break; } }
  ...
  try{
    const r = await fetch(url, { headers: { 'User-Agent': 'BronxUltra/5.6' }});
...
app.post('/api/payment/submit', async (req, res) => {
  ...
  var plan = null;
  for (var i = 0; i < plans.length; i++){ if (plans[i].id === b.planId){ plan = plans[i]; break; } }
...
app.listen(CFG.PORT, () => {
  console.log('════════════════════════════════════════════════════');
  console.log('  BRONX ULTRA v5.6 · @BRONX_ULTRA');
  console.log('  Port: ' + CFG.PORT);
  console.log('  Storage: ' + ((CFG.KV_URL && CFG.KV_TOKEN) ? 'UPSTASH REDIS' : 'IN-MEMORY'));
  console.log('  Login: Gmail + Password (no OAuth required)');
  console.log('════════════════════════════════════════════════════');
});
